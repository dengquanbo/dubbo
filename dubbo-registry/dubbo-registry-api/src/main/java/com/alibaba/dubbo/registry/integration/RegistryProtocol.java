...
public class RegistryProtocol implements Protocol {
    ...
    private final Map<String, ExporterChangeableWrapper<?>> bounds = new ConcurrentHashMap<String,
    ...
    private URL getRegistedProviderUrl(final Invoker<?> originInvoker) {
        ...
        final URL registedProviderUrl =
                providerUrl.removeParameters(getFilteredKeys(providerUrl)).removeParameter(Constants.MONITOR_KEY);
    ...
    public <T> Invoker<T> refer(Class<T> type, URL url) throws RpcException {
        // 从 URL 中获取 registry 参数，并将其设置到 protocol 属性中，并移除
    ...
    private <T> Invoker<T> doRefer(Cluster cluster, Registry registry, Class<T> type, URL url) {
        ...
        directory.setProtocol(protocol); // 塞入动态生成的 Protocol$Adaptive
        // 生成服务消费者链接
        ...
        // 再订阅 注册中心的 providers 目录，configurations 目录 和 routers 目录，订阅好了之后就会触发 DubboProtocol 的 refer 方法
        directory.subscribe(subscribeUrl.addParameter(Constants.CATEGORY_KEY,
                Constants.PROVIDERS_CATEGORY
                        + "," + Constants.CONFIGURATORS_CATEGORY
                        + "," + Constants.ROUTERS_CATEGORY));
        return cluster.join(directory); // 创建 Invoker 对象，利用 cluster 集群封装 directory 其实就是封装多个 invoker
    }
